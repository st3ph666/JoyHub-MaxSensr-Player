...
class BLEWorker:
    ...
    async def _run(self):
        ...
        start_ms = max(0, int(self.settings.get("start_ms", 0) or 0))
        if start_ms > 0:
            cmd.append(f"--start={start_ms / 1000.0:.3f}")
...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("JoyHub MaxSensr Funscript Player v1.4.15 — Persistent Folder + Resume")
        ...
        self.last_folder = str(VIDEO_DIR if VIDEO_DIR.is_dir() else Path.home())
        self.last_position_save_time = 0.0
        self.saved_video_on_startup = None
    ...
    def _vars(self):
        ...
        self.resume_video = tk.BooleanVar(value=True)
    ...
    def change_language(self, *_):
        ...
        for child in list(self.winfo_children()):
            child.destroy()
        self.build_ui()
        self.title("JoyHub MaxSensr Funscript Player v1.4.15 — Persistent Folder + Resume + FR/EN")
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="v1.4.15 — Persistent Folder + Resume", style="Subtitle.TLabel").grid(
        ...
        browse_buttons = ttk.Frame(file_card, style="Card.TFrame")
        browse_buttons.grid(row=1, column=2, pady=2)
        ttk.Button(browse_buttons, text=self.tr("video"), style="Secondary.TButton",
                   command=self.choose_video).pack(side="left", padx=(0, 4))
        ttk.Button(browse_buttons, text=self.tr("folder"), style="Secondary.TButton",
        ...
        ttk.Checkbutton(opts, text=self.tr("delete_after"), variable=self.delete_after,
                        style="Dark.TCheckbutton").pack(side="left", padx=(0, 15))
        ttk.Checkbutton(opts, text=self.tr("resume_video"), variable=self.resume_video,
    ...
    def _set_current_video(self, video, announce=True):
        ...
        if script:
            ...
            if announce:
                self.status_text.set(
                    f"Ready: {video.name}" if self._lang() == "en"
                    else f"Prêt : {video.name}"
                )
            self.save_config()
            return True
        ...
        self.script_path.set(
            f"✕  No script for {video.name}" if self._lang() == "en"
            else f"✕  Aucun script pour {video.name}"
        )
        self.graph_actions = []
        self.graph_duration_ms = 0
        self.draw_funscript_graph()
    ...
    def _folder_videos(self, folder):
        folder = Path(folder)
    ...
    def choose_folder(self):
        p = filedialog.askdirectory(
            initialdir=self.last_folder if Path(self.last_folder).is_dir() else str(Path.home()),
            title="Choose video folder" if self._lang() == "en" else "Choisir le dossier de vidéos"
        )
        if not p:
            return
        folder = Path(p)
        ...
        if not videos:
            messagebox.showwarning(
                "Empty folder" if self._lang() == "en" else "Dossier vide",
                "No compatible videos in this folder." if self._lang() == "en"
                else "Aucune vidéo compatible dans ce dossier."
            )
            self.save_config()
            return

        ready = [v for v in videos if find_script(v)]
        if ready:
            self._set_current_video(ready[0], announce=False)
            self.status_text.set(
                f"Folder loaded: {folder.name} — {len(ready)}/{len(videos)} video(s) with funscript."
                if self._lang() == "en"
                else f"Dossier chargé : {folder.name} — {len(ready)}/{len(videos)} vidéo(s) avec funscript."
            )
            self.save_config()
            return

        messagebox.showwarning(
            "No funscript" if self._lang() == "en" else "Aucun funscript",
            "Videos were found, but none has a matching funscript."
            if self._lang() == "en"
            else "Des vidéos ont été trouvées, mais aucune ne possède de funscript correspondant."
        )
        self.save_config()
    ...
    def choose_video(self):
        p = filedialog.askopenfilename(
            initialdir=self.last_folder if Path(self.last_folder).is_dir() else str(Path.home()),
            filetypes=[("Videos" if self._lang() == "en" else "Vidéos",
                        "*.mp4 *.mkv *.avi *.mov *.webm *.m4v"),
                       ("All" if self._lang() == "en" else "Tous", "*")]
        )
        if not p:
            return
        video = Path(p)
        self.playlist_folder = video.parent
        if not self._set_current_video(video):
            messagebox.showwarning(
    ...
    def play(self):
        ...
        self.graph_position_ms = start_ms
        self.draw_playhead_only()
        self.save_config()

        play_settings = self.settings()
        play_settings["start_ms"] = start_ms
        self.worker.start(video, script, play_settings)

        if start_ms > 0:
            self.status_text.set(
                f"Resume at {start_ms/1000:.1f} s — BLE connection and MPV launch…"
                if self._lang() == "en"
                else f"Reprise à {start_ms/1000:.1f} s — connexion BLE et lancement de MPV…"
            )
        else:
            self.status_text.set(
                "BLE connection and MPV launch on the right screen…"
                if self._lang() == "en"
                else "Connexion BLE et lancement de MPV sur l’écran de droite…"
            )
    ...
    def next_video(self, auto=False):
        ...
        folder = self.playlist_folder or (video.parent if video else None)
        if not video or not folder or not Path(folder).is_dir():
        ...
        for candidate in videos[idx + 1:]:
            if find_script(candidate):
                self._set_current_video(candidate, announce=False)
                self.status_text.set(
                    f"Next video: {candidate.name}" if self._lang() == "en"
                    else f"Vidéo suivante : {candidate.name}"
                )
                if auto:
    ...
    def settings(self):
        return {
            ...
            "resume_video": self.resume_video.get(),
    ...
    def save_config(self):
        ...
        try:
            ...
            data = self.settings() | {
                ...
                "video": str(self.current_video) if self.current_video else self.video_path.get(),
                "script": str(self.current_script) if self.current_script else self.script_path.get().removeprefix("✓  ").strip(),
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"[MaxSensr] Erreur lecture config {CONFIG}: {exc}", file=sys.stderr)
            return

        self.last_folder = data.get("last_folder") or self.last_folder
        pf = data.get("playlist_folder", "")
        ...
        if video_text:
            ...
            if video.is_file():
                self.saved_video_on_startup = video
    ...
    def _restore_last_video(self):
        ...
        video = Path(video)
        if self.playlist_folder is None or not Path(self.playlist_folder).is_dir():
            self.playlist_folder = video.parent

        if self._set_current_video(video, announce=False):
            try:
                saved_ms = int(self.resume_positions.get(str(video.resolve()), 0) or 0)
            except Exception:
                saved_ms = 0

            self.graph_position_ms = saved_ms if self.resume_video.get() else 0
            self.draw_playhead_only()

            if self.resume_video.get() and saved_ms > 0:
                self.status_text.set(
                    f"Last video restored: {video.name} — resume available at {saved_ms/1000:.1f} s."
                    if self._lang() == "en"
                    else f"Dernière vidéo restaurée : {video.name} — reprise disponible à {saved_ms/1000:.1f} s."
                )
            else:
                self.status_text.set(
                    f"Last video restored: {video.name}" if self._lang() == "en"
                    else f"Dernière vidéo restaurée : {video.name}"
                )
        else:
            self.status_text.set(
                f"Last video found, but its funscript is missing: {video.name}"
                if self._lang() == "en"
                else f"Dernière vidéo retrouvée, mais son funscript est introuvable : {video.name}"
            )
